...
from pyhmmer.easel import SSIReader, TextSequence, TextSequenceBlock, DigitalSequenceBlock, Alphabet
...
class FileBuildStrategy(ABC):
    def __init__(self, format: str, result: Result, db_conf: DatabaseSettings):
        self.format = format
        self.result = result
...
class FastaBuildStrategy(FileBuildStrategy):
    def __init__(self, format: str, result: Result, db_conf: DatabaseSettings):
        self.full_length = format.startswith("full")

        super().__init__(format, result, db_conf)
...
class MSABuildStrategy(FileBuildStrategy):
    def __init__(self, query: str, format: str, result: Result, db_conf: DatabaseSettings):
        self.query = query
        super().__init__(format, result, db_conf)

    def build(self, path: os.PathLike):
        match = re.match(r"^>(.*$)\n([\s\S]+)", self.query, re.MULTILINE)

        if match is None:
            raise ValueError(f"Input query is faulty: {self.query}")
        ...
        msa = hmmalign(hmm, self.fetch_sequences())

        with self.open(path, mode="wb") as fh:
            msa.write(fh, self.format)
